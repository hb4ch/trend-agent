...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Phase 1: Market Intelligence        ────→  市场主线白名单           │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ • Web Search: 提取新闻热点、政策催化                          │   │
│  │ • Dragon Tiger List: 分析资金流向、游资偏好                  │   │
│  │ • Multi-Source Fusion: DeepSeek融合双源数据，验证主线         │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                              ↓                                      │
│  Phase 2: Quantitative Mining      ────→  技术面候选股票池         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ • 市值过滤: 20-300亿                                          │   │
│  │ • 横盘筛选: 120日箱体振幅 < 35%                               │   │
│  │ • 均线分析: MA20/60/120 粘合度 < 15%                          │   │
│  │ • 量能信号: 近期放量 1.2-3.0倍                                │   │
│  │ • Qwen语义匹配: 股票-题材归属验证                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                              ↓                                      │
│  Phase 3: Deep Research (Audit)     ────→  尽调结论 + 排雷         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ • 多轮检索: 验证题材真实性、查找监管信息                        │   │
│  │ • 一票否决: 立案/诉讼/减持/退市风险/伪概念                     │   │
│  │ • 时效性判断: 监管事件仅近2年有效                               │   │
│  │ • 优先级: cninfo.com.cn > 交易所 > 其他                       │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                              ↓                                      │
│  Phase 4: Visualization             ────→  K线图表 + 技术信号       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ • mplfinance生成K线图                                        │   │
│  │ • MA20/60/120均线叠加                                         │   │
│  │ • 量能异动点标注 (^ 标记)                                      │   │
...
│  └─────────────────────────────────────────────────────────────┘   │
│                              ↓                                      │
│  Phase 5: Report Generation        ────→  研究报告 (MD + PDF)      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ • DeepSeek V3.2 生成结构化研报                                │   │
│  │ • 工具调用: web_search / duckdb / python                      │   │
│  │ • 彩色标注: 投资逻辑(蓝) / 资金验证(紫) / 催化(红) / 建议(绿)  │   │
│  │ • Pandoc + XeLaTeX 输出中文PDF                                │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
